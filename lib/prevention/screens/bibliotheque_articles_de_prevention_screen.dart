...
import 'package:fr_cnamts_ens/prevention/screens/labels/prevention_profile_type_labels.dart';
...
import 'package:fr_cnamts_ens/prevention/screens/viewmodels/bibliotheque_articles_de_prevention_screen_view_model.dart';
...
class BibliothequeArticlesDePreventionScreen extends StatelessWidget {
  static const routeName = '/bibliotheque-articles-de-prevention';

  const BibliothequeArticlesDePreventionScreen();

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const EnsAppBarSubLevel(title: 'Articles de prévention'),
      body: StoreConnector<EnsState, BibliothequeArticlesDePreventionScreenViewModel>(
        converter: (store) => BibliothequeArticlesDePreventionScreenViewModel.from(store),
        distinct: true,
        onInitialBuild: (vm) {
          vm.fetchPreventionData(false);
          context.tagAction(TagsPrevention.tag_2676_bibliotheque_articles_prevention);
        },
        builder: (context, vm) {
          switch (vm.articleDePreventionScreenStatus) {
...
class _Success extends StatefulWidget {
  final BibliothequeArticlesDePreventionScreenViewModel vm;
          ...
          children: [
            Padding(
              ...
              child: Column(
                children: [
                  Text(
                    ProfilTypeText.fromProfilType(widget.vm.profilType).header,
...
class _HabitudesDeVieCard extends StatelessWidget {
  final ProfilType profilType;

  const _HabitudesDeVieCard({required this.profilType});
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  ProfilTypeText.fromProfilType(profilType).hdvCard,
                  style: EnsTextStyle.text14_w400_normal_body,
                ),
                const EnsLinkText(
                  label: 'Voir les habitudes de vie',
      ...
      onTap: () {
        ...
        if (hasDetailArticle) {
          Navigator.pushNamed(
            ...
            arguments: PreventionDetailScreenArgument(articleId: id, isGenerique: true),
          );
